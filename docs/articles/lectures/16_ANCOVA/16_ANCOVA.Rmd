...
The regression line $y = \beta_0 + \beta_1$ represents the mean $y$ response at a given $x$ value  
...
One way to write the model involves an intercept ( $\large \beta_0$ ) for one factor level and $\large \beta$'s that describe the difference between the intercepts of the other factors and the reference level  
