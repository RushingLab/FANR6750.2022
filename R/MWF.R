MWF <- function(first.day, last.day){
  ...
  if(lubridate::wday(first.day) == 2){
    ...
    Fd <- lubridate::day(seq(as.Date(first.day) + 4, last.f, "7 days"))
    Fm <- lubridate::month(seq(as.Date(first.day) + 4, last.f, "7 days"), label = TRUE)

    M <- paste(Mm, Md, sep = ". ")
    W <- paste(Wm, Wd, sep = ". ")
    Fr <- paste(Fm, Fd, sep = ". ")

    dates <- c(rbind(M,W,Fr, rep("", length(M))))
    ...
    Fd <- lubridate::day(seq(first.m + 4, last.f, "7 days"))
    Fm <- lubridate::month(seq(first.m + 4, last.f, "7 days"), label = TRUE)
  ...
  if(last.wday %in% c(2,3)){
    dates <- dates[-c(length(dates) - 1, length(dates))]
  }
